...
public class EmployeeController {
    ...
    public ResponseEntity<CustomApiResponse<EmployeeDto>> createEmployee(@Valid @RequestBody CreateEmployeeRequest request) {
        EmployeeDto createdEmployee = employeeService.createEmployee(request);
        return new ResponseEntity<>(CustomApiResponse.<EmployeeDto>builder().data(createdEmployee).build(), HttpStatus.CREATED);
    }
    ...
    @PutMapping("/{id}")
    @PreAuthorize("hasRole('ADMIN') or hasRole('HR')")
    public ResponseEntity<CustomApiResponse<EmployeeDto>> updateEmployee(@PathVariable Long id, @Valid @RequestBody UpdateEmployeeRequest request) {

...
public class LeaveController {
    ...
    public ResponseEntity<CustomApiResponse<LeaveRequestDto>> createLeaveRequest(@Valid @RequestBody CreateLeaveRequest request) {
        LeaveRequestDto createdRequest = leaveService.createLeaveRequest(request);
        return new ResponseEntity<>(CustomApiResponse.<LeaveRequestDto>builder().data(createdRequest).build(), HttpStatus.CREATED);
    }
    ...
    @PreAuthorize("hasRole('ADMIN') or hasRole('HR') or hasRole('MANAGER') or hasRole('EMPLOYEE')")
    public ResponseEntity<CustomApiResponse<List<LeaveRequestDto>>> getLeaveRequestsByEmployee(@PathVariable Long employeeId) {
